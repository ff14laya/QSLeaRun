...
namespace Learun.Application.Web.Areas.LR_AuthorizeModule.Controllers
{
    ...
    public class DataAuthorizeController : MvcControllerBase
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult CustomerFormIndex()
        {
            return View();
        }
        ...
        /// <summary>
        /// <returns></returns>
        public ActionResult GetRelationPageList(string pagination, string interfaceId, string keyword, string objectId,int type)
        {
            Pagination paginationobj = pagination.ToObject<Pagination>();
            var data = dataAuthorizeIBLL.GetRelationPageList(paginationobj, interfaceId, keyword, objectId,type);
            var jsonData = new
            {
                rows = data,
