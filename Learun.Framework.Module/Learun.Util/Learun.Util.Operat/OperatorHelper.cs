...
namespace Learun.Util.Operat
{
    ...
    public class OperatorHelper
    {
        ...
        private string cacheKeyInfo = "learun_adms_info_";// + Mark
        /// <summary>
        /// 秘钥
        /// </summary>
        ...
        public void EmptyCurrent(string token, string loginMark)
        {
            try
            {
                ...
                if (operatorInfo != null && operatorInfo.token == token)
                {
                    ...
                    redisCache.Remove(cacheKeyInfo + operatorInfo.account, CacheId.loginInfo);
        ...
        public OperatorResult IsOnLine(string account)
        {
            try
            {
                string token = WebHelper.GetCookie(LoginUserToken).ToString();
                string loginMark = WebHelper.GetCookie(LoginUserMarkKey).ToString();
                return IsOnLine(token, loginMark, account);
            }
            catch (Exception)
            {
        ...
        /// <returns>-1未登录,1登录成功,0登录过期</returns>
        public OperatorResult IsOnLine(string token, string loginMark,string account = "")
        {
            ...
            try
            {
                ...
                if (operatorInfo != null && operatorInfo.token == token)
                {
                    ...
                    if (operatorInfo.appId != "App" && span.TotalHours >= 12)// 登录操作过12小时移除(App登录除外)
                    {
                    ...
                    else
                    {
                        ...
                        operatorResult.userInfo.token = operatorInfo.token;
                    }
                }
                return operatorResult;
            }
