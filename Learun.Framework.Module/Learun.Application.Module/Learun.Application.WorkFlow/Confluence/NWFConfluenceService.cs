...
namespace Learun.Application.WorkFlow
{
    ...
    public class NWFConfluenceService : RepositoryFactory
    {
        ...
        public IEnumerable<NWFConfluenceEntity> GetList(string processId, string nodeId)
        {
            try
            {
                return  this.BaseRepository().FindList<NWFConfluenceEntity>(t => t.F_ProcessId == processId && t.F_NodeId == nodeId);
            }
            catch (Exception ex)
            {
        ...
        public void DeleteEntity(string processId, string nodeId)
        {
            try
            {
                this.BaseRepository().Delete<NWFConfluenceEntity>(t => t.F_ProcessId == processId && t.F_NodeId == nodeId);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
