...
namespace Learun.Application.TwoDevelopment.SystemDemo
{
    ...
    public class DemoleaveService : RepositoryFactory
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<DemoleaveEntity> GetList()
        {
            try
            {
                return this.BaseRepository().FindList<DemoleaveEntity>();
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void SaveList(string list)
        {
            try
            {
                List<DemoleaveEntity> listData = list.ToList<DemoleaveEntity>();
                foreach (DemoleaveEntity item in listData)
                {
