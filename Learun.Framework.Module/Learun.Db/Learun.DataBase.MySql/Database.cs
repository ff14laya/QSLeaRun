            ...
            try
            {
                ...
                strSql.Append(@"
                             SELECT TABLE_NAME name,0 reserved,0 data,0 index_size,TABLE_ROWS sumrows,0 unused, (select IF(LENGTH(TRIM(TABLE_COMMENT))<1,TABLE_NAME,TABLE_COMMENT)) tdescription
                               ,'' pk from information_schema.`TABLES` where TABLE_SCHEMA =database() ORDER BY TABLE_NAME
                            ");
                strSql2.Append("select COLUMN_NAME pk,TABLE_NAME name from information_schema.`COLUMNS` where TABLE_SCHEMA=database() and COLUMN_KEY='PRI'");
                var table = dbcontext.Database.Connection.Query(strSql.ToString(), null, dbTransaction);
                var pk = dbcontext.Database.Connection.Query(strSql2.ToString(), null, dbTransaction);
                ...
                foreach (var item in pk) {
                    ...
                    {
                        dic[item.name] = dic[item.name] + "," + item.pk;
                    }
                    else {
                        dic.Add(item.name, item.pk);
                    }
                }

                foreach (var item in table) {
                    if (dic.ContainsKey(item.name)) {
                        item.pk = dic[item.name];
                    }
                       
                }

                return table.ToJson().ToObject<List<T>>();
            }
            catch (Exception)
            {
